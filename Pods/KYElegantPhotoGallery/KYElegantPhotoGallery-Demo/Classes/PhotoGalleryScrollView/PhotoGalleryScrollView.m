...
-(void)setUp:(NSMutableArray *)imageViewArray  frame:(CGRect)frame {
    ...
    @autoreleasepool {
        for (int i=0; i<imageViewArray.count; i++) {
            ...
            image.bounds = CGRectMake(0, 0, SCREENWIDTH, frame.size.height <= (igv.image.size.height*SCREENWIDTH/igv.image.size.width)?frame.size.height:(igv.image.size.height*SCREENWIDTH/igv.image.size.width));
